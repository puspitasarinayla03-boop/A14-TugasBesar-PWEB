...
async function verifyToken(req, res, next) {
  try {
    ...
    const [rows] = await query(
      'SELECT id, name, email, role_id, is_active FROM users WHERE id = ? LIMIT 1',
      [decoded.id]
    );
    ...
    if (!rows.length || !rows[0].is_active) {
      if (req.originalUrl.startsWith('/api/')) {
        ...
        // Clear cookie if user is inactive
  ...
  return (req, res, next) => {
    ...
    if (!allowedRoles.includes(req.user.role)) {
      if (req.originalUrl.startsWith('/api/')) {
        ...
        return res.status(403).render('403', { title: 'Akses Ditolak', user: req.user });
      }
    }
